...
define pvector
	if $argc == 0
		...
		set $capacity = $arg0.__end_ - $arg0.__begin_
		set $size_max = $size - 1
	end
	...
	if $argc == 2
 	...
 	if $argc > 0
    if $size > 1000
        printf "do not be silly: size=%u\n", $size
    else
        set $i = 0
        set $mymin = 9999999999
        set $mymax = -1
        set $mypositiveval=0
        set $myzeroval=0
        set $mynegativeval=0
     	while $i < $size
            set $myval = $arg0.__begin_[$i]
            # printf "%u %f\n", $i, $myval
            # if $arg0.__begin_[$i] < $mymin

            # --- minimum:
            if $myval < $mymin
                # printf "if: %u x=%u min=%u\n", $i, $arg0.__begin_[$i], $mymin
                set $mymin = $myval
                # set $mymin = $arg0.__begin_[$i]
                # printf "if: %u x=%u min=%u\n", $i, $arg0.__begin_[$i], $mymin
            end

            # --- maximum:
            # if $arg0.__begin_[$i] > $mymax
            if $myval > $mymax
                set $mymax = $myval
                # set $mymax = $arg0.__begin_[$i]
            end
        ...
        printf "min=%f max=%f ", $mymin, $mymax
        printf ">0=%d 0=%d <0=%d\n", $mypositiveval, $myzeroval, $mynegativeval
    end
    end
...
# 			set $i++
#             if $arg0.__begin_[$i] > $mymax
#                 # printf "if: i=%u elem=%u is > max=%u -> max\n", $i, $arg0.__begin_[$i], $mymax
#                 set $mymax = $arg0.__begin_[$i]
#                 # printf "# mymax=%u i=%u size=%u\n", $mymax, $i, $size
# 			# printf "xxx elem[%u]: ", $i
# 			# p $arg0.__begin_[$i]
#             # else
#             #     set $mynull = 0
#             #     # printf "else: i=%u elem=%u is NOT > max=%u \n", $i, $arg0.__begin_[$i], $mymax
# 		end
# 	end
#     # printf "max=%u\n", $mymax
#     # printf "Element "
