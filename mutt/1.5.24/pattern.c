...
/* ================================= */
int mutt_pattern_func_cscs (int op, char *prompt, int cscs_menu_current)
{
  pattern_t *pat;
...
/* CSCS 
        FILE *filejg;
        filejg=fopen("/apps/pilatus/sandbox/jgp/mutt/src/mutt-1.5.21/muttjg2","w");
*/
        char delim='#';
        char* s2="";
        char s3[5]="";
        s2=strchr(Context->hdrs[Context->v2r[cscs_menu_current]]->env->subject , delim ); 
        s2++;
        strncpy(s3, s2, 5);
        s3[5]='\0';
/*
        fprintf(filejg,"pattern.c\ns1=%s i=%d\n",Context->hdrs[Context->v2r[cscs_menu_current]]->env->subject,cscs_menu_current);
        fprintf(filejg,"cscs_menu_current=%d \n",cscs_menu_current);
        fprintf(filejg,"s2=%s \n",s2);
        fprintf(filejg,"s3=%s \n",s3);
        fclose(filejg);
*/
...
/* ================================= */
int mutt_pattern_func_cscs_resolved (int op, char *prompt)
{
  pattern_t *pat;
  char buf[LONG_STRING] = "", *simple, error[STRING];
  BUFFER err;
...
/* ================================= */
/* see protos.h and functions.h */
/* ================================= */
int mutt_pattern_func_cscs_all (int op, char *prompt)
{
  pattern_t *pat;
  char buf[LONG_STRING] = "", *simple, error[STRING];
  BUFFER err;
