...
// {{
	...
	menu->oldcurrent = (Context->vcount && menu->current >= 0 && menu->current < Context->vcount) ?
		CURHDR->index : -1;
	//cscs if (mutt_pattern_func (M_LIMIT, _("Limit to messages matching: ")) == 0)
	if (mutt_pattern_func_cscs (M_LIMIT, _("Limit to messages matching: "), menu->current) == 0) 
	{
...
// ---- cscs calling OP_MAIN_DELETE_PATTERN immediatly after "l" (=limit)
	CHECK_MSGCOUNT;
        CHECK_VISIBLE;
	CHECK_READONLY;
	CHECK_ACL(M_ACL_DELETE, _("delete message(s)"));
...
// }}
// {{
//cscs         if ((op == OP_MAIN_CHANGE_FOLDER_READONLY) || option (OPTREADONLY))
//cscs           cp = _("Open mailbox in read-only mode");
//cscs         else
//cscs           cp = _("Open mailbox");
//cscs 
//cscs 	buf[0] = '\0';
//cscs 	if ((op == OP_MAIN_NEXT_UNREAD_MAILBOX) && Context && Context->path)
//cscs 	{
//cscs 	  strfcpy (buf, Context->path, sizeof (buf));
//cscs 	  mutt_pretty_mailbox (buf, sizeof (buf));
//cscs 	  mutt_buffy (buf, sizeof (buf));
//cscs 	  if (!buf[0])
//cscs 	  {
//cscs 	    mutt_error _("No mailboxes have new mail");
//cscs 	    break;
//cscs 	  }
//cscs 	}
//cscs 	else
...
//cscs 	  mutt_buffy (buf, sizeof (buf));
//cscs 
//cscs 	  if (mutt_enter_fname (cp, buf, sizeof (buf), &menu->redraw, 1) == -1)
//cscs 	  {
//cscs 	    if (menu->menu == MENU_PAGER)
//cscs 	    {
//cscs 	      op = OP_DISPLAY_MESSAGE;
//cscs 	      continue;
//cscs 	    }
//cscs 	    else
//cscs 	      break;
...
//cscs 	  if (!buf[0])
//cscs 	  {
//cscs 	    CLEARLINE (LINES-1);
//cscs 	    break;
//cscs 	  }
//cscs 	}
//cscs 
//cscs 	mutt_expand_path (buf, sizeof (buf));
//cscs 	if (mx_get_magic (buf) <= 0)
//cscs 	{
//cscs 	  mutt_error (_("%s is not a mailbox."), buf);
//cscs 	  break;
//cscs 	}
//cscs 	mutt_str_replace (&CurrentFolder, buf);
//cscs 
//cscs 	/* keepalive failure in mutt_enter_fname may kill connection. #3028 */
//cscs 	if (Context && !Context->path)
//cscs 	  FREE (&Context);
//cscs 
//cscs         if (Context)
//cscs         {
//cscs 	  int check;
//cscs 
//cscs 	  mutt_str_replace (&LastFolder, Context->path);
//cscs 	  oldcount = Context ? Context->msgcount : 0;
//cscs 
//cscs 	  if ((check = mx_close_mailbox (Context, &index_hint)) != 0)
//cscs 	  {
//cscs 	    if (check == M_NEW_MAIL || check == M_REOPENED)
//cscs 	      update_index (menu, Context, check, oldcount, index_hint);
//cscs 
//cscs 	    set_option (OPTSEARCHINVALID);
//cscs 	    menu->redraw = REDRAW_INDEX | REDRAW_STATUS;
//cscs 	    break;
//cscs 	  }
//cscs 	  FREE (&Context);
//cscs 	}
...
//cscs         mutt_sleep (0);
//cscs 
//cscs 	/* Set CurrentMenu to MENU_MAIN before executing any folder
//cscs 	 * hooks so that all the index menu functions are available to
//cscs 	 * the exec command.
//cscs 	 */
//cscs 
//cscs 	CurrentMenu = MENU_MAIN;
//cscs 	mutt_folder_hook (buf);
//cscs 
//cscs 	if ((Context = mx_open_mailbox (buf,
//cscs 					(option (OPTREADONLY) || op == OP_MAIN_CHANGE_FOLDER_READONLY) ?
//cscs 					M_READONLY : 0, NULL)) != NULL)
//cscs 	{
//cscs 	  menu->current = ci_first_message ();
//cscs 	}
//cscs 	else
//cscs 	  menu->current = 0;
//cscs 
//cscs 	mutt_clear_error ();
//cscs 	mutt_buffy_check(1); /* force the buffy check after we have changed
//cscs 			      the folder */
//cscs 	menu->redraw = REDRAW_FULL;
//cscs 	set_option (OPTSEARCHINVALID);
//cscs 	break;
// }}
...
//cscs 	CHECK_MSGCOUNT;
//cscs         CHECK_VISIBLE;
//cscs 	if (option (OPTPGPAUTODEC) && (tag || !(CURHDR->security & PGP_TRADITIONAL_CHECKED)))
//cscs 	  mutt_check_traditional_pgp (tag ? NULL : CURHDR, &menu->redraw);
//cscs 	ci_send_message (SENDREPLY|SENDLISTREPLY, NULL, NULL, Context, tag ? NULL : CURHDR);
//cscs 	menu->redraw = REDRAW_FULL;
//cscs 	break;
*/
	CHECK_IN_MAILBOX;
	menu->oldcurrent = (Context->vcount && menu->current >= 0 && menu->current < Context->vcount) ?
		CURHDR->index : -1;
	if (mutt_pattern_func_cscs_all (M_LIMIT, _("Limit to messages matching: ")) == 0)
...
//cscs 	CHECK_MSGCOUNT;
//cscs 	CHECK_VISIBLE;
//cscs 	mutt_print_message (tag ? NULL : CURHDR);
//cscs 
//cscs #ifdef USE_IMAP
//cscs 	/* in an IMAP folder index with imap_peek=no, printing could change
//cscs 	 * new or old messages status to read. Redraw what's needed.
//cscs 	 */
//cscs 	if (Context->magic == M_IMAP && !option (OPTIMAPPEEK))
//cscs 	{
//cscs 	  menu->redraw = (tag ? REDRAW_INDEX : REDRAW_CURRENT) | REDRAW_STATUS;
//cscs 	}
//cscs #endif
...
//cscs 	break;
	...
	menu->oldcurrent = (Context->vcount && menu->current >= 0 && menu->current < Context->vcount) ?
		CURHDR->index : -1;
	if (mutt_pattern_func_cscs_resolved (M_LIMIT, _("Limit to messages matching: ")) == 0)
